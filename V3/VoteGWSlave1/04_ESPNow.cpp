...
char ESPNOW::DuLieuGuiToiMasTer[DATA_SIZE];
...
std::vector<std::string> ESPNOW::FIFODataCuaNodeGuiLenMaster;
...
void ESPNOW::DuLieuNhanTuNode(uint8_t* MAC, uint8_t* DuLieu, uint8_t ChieuDaiDuLieu) {
  if (*DuLieu != 13) {
    ...
    ESPNOW::FIFODataCuaNodeGuiLenMaster.push_back(DuLieuGuiToiMasTer);

    for (const std::string& str : ESPNOW::FIFODataCuaNodeGuiLenMaster) {
